...
Route::group(['middleware' => ['web', 'admin.user']], function () {
    ...
    Route::post('/admin/menu/add_item', function (Request $request) {
        $data = $request->all();
        $highest_order_menu_item = TCG\Voyager\Models\MenuItem::where('parent_id', '=', null)->orderBy('order',
            'DESC')->first();
    ...
    Route::get('/admin/database/create-table', [
        'as' => 'voyager_database_create_table',
        'uses' => 'TCG\Voyager\Controllers\VoyagerDatabaseController@create'
    ]);
