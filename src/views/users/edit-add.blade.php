        ...
        <form role="form" action="@if(isset($dataTypeContent->id)){{ '/admin/' . $dataType->slug . '/' . $dataTypeContent->id }}@else{{ '/admin/' . $dataType->slug }}@endif" method="POST" enctype="multipart/form-data">
          <div class="panel-body">
            ...
            @foreach($dataType->addRows as $row)
              <div class="form-group">
                ...
                @elseif($row->type == "radio_btn")
                  ...
                  <?php $default = (isset($options->default) && !isset($dataTypeContent->{$row->field})) ? $options->default : NULL; ?>
                    ...
                    @if(isset($options->options))
                        @foreach($options->options as $key => $option)
                          <li>
                            <input type="radio" id="option-{{ $key }}" name="{{ $row->field }}" value="{{ $key }}" @if($default == $key && $selected_value === NULL){{ 'checked' }}@endif @if($selected_value == $key){{ 'checked' }}@endif>
                ...
                @elseif($row->type == "checkbox")
                  ...
                  @else
                      <input type="checkbox" name="{{ $row->field }}" class="toggleswitch" @if($checked) checked @endif>
                  @endif
            ...
            <div class="form-group">
                ...
                <select class="user-roles form-control" name="roles[]" multiple="multiple">
                  @foreach(TCG\Voyager\Models\Role::all() as $role)
                    <option value="{{$role->id}}" @if(isset($dataTypeContent) && $dataTypeContent->hasRole($role->name)) selected="selected" @endif>{{$role->display_name}}</option>
                  @endforeach
                </select>
            </div>
