    ...
    <table class="w-full text-white mt-6">
        ...
        <tbody>
            <tr v-for="(rule, i) in rules" v-bind:key="'rule-'+i">
                <th>
                    <input v-model="rule.rule" class="voyager-input small" :placeholder="__('voyager::manager.rule')">
                </th>
                <th>
                    <language-input
                ...
                <th>
                    <button class="button red small" @click="removeRule(i)">{{ __('voyager::generic.delete') }}</button>
                </th>
            </tr>
        </tbody>
        <tfoot>
            <button class="button green small mt-6" @click="addRule">{{ __('voyager::generic.add') }}</button>
        </tfoot>
    </table>
</template>
