                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            <td>
                                                ...
                                                @elseif($row->type == 'select_dropdown')
                                                    @if(property_exists($options, 'options') && property_exists($options->options, $data->{$row->field}))
                                                        ...
                                                        @else
                                                            {!! $options->options->{$data->{$row->field}} !!}
                                                        @endif
                                                    @elseif($data->{$row->field . '_page_slug'})
                                                        <a href="{{ $data->{$row->field . '_page_slug'} }}">
                                                            {{ $data->{$row->field} }}
                                                        </a>
                                                    @else
                                                ...
                                                @elseif($row->type == 'text')
                                                    ...
                                                    <div
                                                        class="readmore">{{ strlen( $data->{$row->field} ) > 200 ? substr($data->{$row->field}, 0, 200) . ' ...' : $data->{$row->field} }}</div>
                                                ...
                                                @elseif($row->type == 'rich_text_box')
                                                    @include('voyager::multilingual.input-hidden-bread-browse')
                                                    <div
                                                        class="readmore">{{ strlen( strip_tags($data->{$row->field}, '<b><i><u>') ) > 200 ? substr(strip_tags($data->{$row->field}, '<b><i><u>'), 0, 200) . ' ...' : strip_tags($data->{$row->field}, '<b><i><u>') }}</div>
                                                @else
                                                    @include('voyager::multilingual.input-hidden-bread-browse')
