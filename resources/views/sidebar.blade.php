<aside class="flex flex-col h-full min-h-screen aside-expand w-48">

    <div class="bg-gray-900">
        <div class="flex justify-center m-4  items-center text-white text-lg uppercase">Voyager</div>
    </div>

    <div class="flex-1 bg-gray-800">
        <ul class="list-reset ml-6">

            <li>
                <a href="/admin" class="flex items-center no-underline text-grey-dark hover:text-white">
                <span class="w-4 h-4">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M502.7 265l-80.3-80.4 47.8-47.9c13.1-13.1 13.1-34.4 0-47.5l-47.5-47.5c-13.1-13.1-34.4-13.1-47.5 0l-47.8 47.9-80.3-80.3C240.8 3.1 232.7 0 224.5 0S208.2 3.1 202 9.3L105.3 106c-12.4 12.4-12.4 32.6 0 45.1l80.3 80.4-9.8 9.8C122.1 217 59.6 218.6 7.3 246.7c-8.5 4.6-9.6 16.4-2.8 23.2L112 377.4l-17.8 17.8c-2.6-.7-5-1.6-7.8-1.6-17.7 0-32 14.3-32 32s14.3 32 32 32 32-14.3 32-32c0-2.8-.9-5.2-1.6-7.8l17.8-17.8 107.5 107.5c6.8 6.8 18.7 5.7 23.2-2.8 28.1-52.3 29.7-114.8 5.4-168.5l9.9-9.9 80.3 80.4c6.2 6.2 14.4 9.3 22.5 9.3s16.3-3.1 22.5-9.3l96.7-96.7c12.5-12.4 12.5-32.6.1-45zm-352-136.5l73.8-73.8 68.9 68.9-73.8 73.8-68.9-68.9zm232.8 232.8l-68.9-68.9 73.8-73.8 68.9 68.9-73.8 73.8z"></path></svg>
                </span>
                    <span class="ml-3 font-hairline"> Dashboard </span>
                </a>
            </li>

            <li>
                <a href="/admin/users" class="flex items-center no-underline text-grey-dark hover:text-white">
                    <v-icon name="user-astronaut" class=" fill-current h-4 w-4"></v-icon>
                    <span class="ml-3 font-hairline"> Users </span>
                </a>
            </li>

            <li>
                <a href="/admin/posts" class="flex items-center no-underline text-grey-dark hover:text-white">
                    <v-icon name="newspaper" class=" fill-current h-4 w-4"></v-icon>
                    <span class="ml-3 font-hairline"> Posts </span>
                </a>
            </li>

            <li>
                <a href="/admin/pages" class="flex items-center no-underline text-grey-dark hover:text-white">
                    <v-icon name="file-alt" class=" fill-current h-4 w-4"></v-icon>
                    <span class="ml-3 font-hairline"> Pages </span>
                </a>
            </li>

            <li>
                <a href="/admin/devices" class="flex items-center no-underline text-grey-dark hover:text-white">
                    <v-icon name="laptop" class=" fill-current h-4 w-4"></v-icon>
                    <span class="ml-3 font-hairline"> Managed Devices </span>
                </a>
            </li>

        </ul>
    </div>

</aside>