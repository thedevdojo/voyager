...
@section('page_title', __('voyager::voyager.generic.menu_builder'))

@section('page_header')
    <h1 class="page-title">
        <i class="voyager-list"></i>{{ __('voyager::voyager.generic.menu_builder') }} ({{ $menu->name }})
        <div class="btn btn-success add_item"><i class="voyager-plus"></i> {{ __('voyager::voyager.menu_builder.new_menu_item') }}</div>
    </h1>
            ...
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    <div class="panel-heading">
                        <p class="panel-title" style="color:#777">{{ __('voyager::voyager.menu_builder.drag_drop_info') }}</p>
                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="voyager-trash"></i> {{ __('voyager::voyager.menu_builder.delete_item_question') }}</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('voyager.menus.item.destroy', ['menu' => $menu->id, 'id' => '__id']) }}"
                        ...
                        <input type="submit" class="btn btn-danger pull-right delete-confirm"
                               value="{{ __('voyager::voyager.menu_builder.delete_item_confirm') }}">
                    </form>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('voyager::voyager.generic.close') }}"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 id="m_hd_add" class="modal-title hidden"><i class="voyager-plus"></i> {{ __('voyager::voyager.menu_builder.create_new_item') }}</h4>
                    <h4 id="m_hd_edit" class="modal-title hidden"><i class="voyager-edit"></i> {{ __('voyager::voyager.menu_builder.edit_item') }}</h4>
                </div>
                <form action="" id="m_form" method="POST"
                    ...
                    <div class="modal-body">
                        @include('voyager::multilingual.language-selector')
                        <label for="name">{{ __('voyager::voyager.menu_builder.item_title') }}</label>
                        @include('voyager::multilingual.input-hidden', ['_field_name' => 'title', '_field_trans' => ''])
                        <input type="text" class="form-control" id="m_title" name="title" placeholder="{{ __('voyager::voyager.generic.title') }}"><br>
                        <label for="type">{{ __('voyager::voyager.menu_builder.link_type') }}</label>
                        <select id="m_link_type" class="form-control" name="type">
                            <option value="url" selected="selected">{{ __('voyager::voyager.menu_builder.static_url') }}</option>
                            <option value="route">{{ __('voyager::voyager.menu_builder.dynamic_route') }}</option>
                        </select><br>
                        <div id="m_url_type">
                            <label for="url">{{ __('voyager::voyager.menu_builder.url') }}</label>
                            <input type="text" class="form-control" id="m_url" name="url" placeholder="{{ __('voyager::voyager.generic.url') }}"><br>
                        </div>
                        <div id="m_route_type">
                            <label for="route">{{ __('voyager::voyager.menu_builder.item_route') }}</label>
                            <input type="text" class="form-control" id="m_route" name="route" placeholder="{{ __('voyager::voyager.generic.route') }}"><br>
                            <label for="parameters">{{ __('voyager::voyager.menu_builder.route_parameter') }}</label>
                        ...
                        <label for="icon_class">{{ __('voyager::voyager.menu_builder.icon_class') }} <a
                                    href="{{ route('voyager.compass.index', [], false) }}#fonts"
                                    target="_blank">{!! __('voyager::voyager.menu_builder.icon_class2') !!}</label>
                        <input type="text" class="form-control" id="m_icon_class" name="icon_class"
                               placeholder="{{ __('voyager::voyager.menu_builder.icon_class_ph') }}"><br>
                        <label for="color">{{ __('voyager::voyager.menu_builder.color') }}</label>
                        <input type="color" class="form-control" id="m_color" name="color"
                               placeholder="{{ __('voyager::voyager.menu_builder.color_ph') }}"><br>
                        <label for="target">{{ __('voyager::voyager.menu_builder.open_in') }}</label>
                        <select id="m_target" class="form-control" name="target">
                            <option value="_self" selected="selected">{{ __('voyager::voyager.menu_builder.open_same') }}</option>
                            <option value="_blank">{{ __('voyager::voyager.menu_builder.open_new') }}</option>
                        </select>
                        <input type="hidden" name="menu_id" value="{{ $menu->id }}">
                        <input type="hidden" name="id" id="m_id" value="">
                    </div>
                    <div class="modal-footer">
                        <input type="submit" class="btn btn-success pull-right delete-confirm__" value="{{ __('voyager::voyager.generic.update') }}">
                        <button type="button" class="btn btn-default pull-right" data-dismiss="modal">{{ __('voyager::voyager.generic.cancel') }}</button>
                    </div>
                </form>
            </div><!-- /.modal-content -->
        ...
        $(document).ready(function () {
            ...
            $('.dd').on('change', function (e) {
                $.post('{{ route('voyager.menus.order',['menu' => $menu->id]) }}', {
                    ...
                    toastr.success("{{ __('voyager::voyager.menu_builder.updated_order') }}");
                });
            });
        });
    </script>
