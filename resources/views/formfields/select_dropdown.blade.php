@if(isset($options->relationship))
    ...
    @if( method_exists( $dataType->model_name, \Illuminate\Support\Str::camel($row->field) ) )
        ...
        <select class="form-control select2" name="{{ $options->form_field_name ?? $row->field }}">
    ...
    @else
        <select class="form-control select2" name="{{ $options->form_field_name ?? $row->field }}"></select>
    @endif
@else
