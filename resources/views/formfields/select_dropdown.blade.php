@if(isset($options->relationship))
    ...
    @if( method_exists( $dataType->model_name, \Illuminate\Support\Str::camel($row->field) ) )
        ...
        <select class="form-control select2" name="{{ $row->field }}" {!! outputAriaForHelpterText($row) !!}>
    ...
    @else
        <select class="form-control select2" name="{{ $row->field }}" {!! outputAriaForHelpterText($row) !!}></select>
    @endif
@else
